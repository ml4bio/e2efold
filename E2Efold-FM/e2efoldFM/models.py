...
import sys
sys.path.append('../')
from e2efoldFM.new_common.utils import soft_sign
import numpy as np
from scipy.sparse import diags


class No_Noise(nn.Module):
    def __init__(self, d, L, device):
        super(No_Noise, self).__init__()
        self.d = d
        self.L = L
        ...
        self.conv_test_1 = nn.Conv2d(in_channels=6 * d + 1, out_channels=d, kernel_size=1)
        ...
        # self.conv_test_1 = nn.Conv2d(in_channels=6*d+1, out_channels=4*d, kernel_size=1)
        # self.bn_conv_1 = nn.BatchNorm2d(4*d)
        # self.conv_test_2 = nn.Conv2d(in_channels=4*d, out_channels=2*d, kernel_size=1)
        # self.bn_conv_2 = nn.BatchNorm2d(2*d)
        # self.conv_test_3 = nn.Conv2d(in_channels=2*d, out_channels=d, kernel_size=1)
        # self.bn_conv_3 = nn.BatchNorm2d(d)
        # self.conv_test_4 = nn.Conv2d(in_channels=d, out_channels=1, kernel_size=1)
    ...
    def forward(self, pe, seq, state, embedding):
        """
        prior: L*L*1
        seq: L*4
        state: L*L
        ...
        #print('infor shape: ', infor.shape)
        #print('embedding shape: ', embedding.shape)
        infor = torch.cat([infor, embedding], 1)
        ...
        # contact = F.relu(self.bn_conv_1(self.conv_test_1(infor)))
        # contact = F.relu(self.bn_conv_2(self.conv_test_2(contact)))
        # contact = F.relu(self.bn_conv_3(self.conv_test_3(contact)))
        # contact = self.conv_test_4(contact)
...
class Test_Model_1(nn.Module):
    def __init__(self, d, L, device):
        super(Test_Model_1, self).__init__()
        self.d = d
        self.L = L
        self.conv1d1= nn.Conv1d(in_channels=4, out_channels=d, 
            kernel_size=9, padding=8, dilation=2)
        ...
        self.conv_test_1 = nn.Conv2d(in_channels=6*d, out_channels=4*d, kernel_size=1)
        self.bn_conv_1 = nn.BatchNorm2d(4*d)
        self.conv_test_2 = nn.Conv2d(in_channels=4*d, out_channels=2*d, kernel_size=1)
        self.bn_conv_2 = nn.BatchNorm2d(2*d)
        self.conv_test_3 = nn.Conv2d(in_channels=2*d, out_channels=d, kernel_size=1)
...
class Test_Model_2(nn.Module):
    def __init__(self, d, L):
        ...
        self.bn1 = nn.BatchNorm1d(d)
        self.conv1d2= nn.Conv1d(in_channels=d, out_channels=d, kernel_size=9, padding=8, dilation=2)
        self.bn2 = nn.BatchNorm1d(d)
        ...
        self.lc = LocallyConnected2d(2*d+1, 1, L, 1)
    ...
    def forward(self, prior, seq, state, embedding):
        """
        state: L*L*1
        seq: L*4
        """
...
class Test_Model_4(nn.Module):
    ...
    def forward(self, pe, seq, state, embedding):
        ...
        infor = torch.cat([seq_mat, p_mat], 1) # 6d*L*L
        embedding = torch.unsqueeze(embedding, 1)
        # embedding = torch.logit(embedding)
        # data_noise = torch.randn(embedding.shape, device=device)*0.25 + 0
        # embedding = embedding + data_noise
        # m = nn.Sigmoid()
        # embedding = m(embedding)
        infor = torch.cat([infor, embedding], 1)
        # print(self.conv_test_1.weight.device)
        
        noise_1 = torch.randn(self.conv_test_1.weight.size(), device=device)*0.3 + 0
        noise_2 = torch.randn(self.conv_test_2.weight.size(), device=device)*0.3 + 0
        noise_3 = torch.randn(self.conv_test_3.weight.size(), device=device)*0.3 + 0
        # print(noise_3.device)
        # exit(1)
        self.add_noise(self.conv_test_1.weight, noise_1)
        contact = F.relu(self.bn_conv_1(self.conv_test_1(infor)))
        self.add_noise(self.conv_test_2.weight, noise_2)
        contact = F.relu(self.bn_conv_2(self.conv_test_2(contact)))
        self.add_noise(self.conv_test_3.weight, noise_3)
        contact = self.conv_test_3(contact)
...
class Test_Model_5(nn.Module):
    ...
    def forward(self, pe, seq, state, embedding):
        ...
        noise_1 = torch.randn(self.conv_test_1.weight.size(), device=device)*0.2 + 0
        noise_2 = torch.randn(self.conv_test_2.weight.size(), device=device)*0.2 + 0
        noise_3 = torch.randn(self.conv_test_3.weight.size(), device=device)*0.2 + 0
        # print(noise_3.device)
        # exit(1)
        self.add_noise(self.conv_test_1.weight, noise_1)
        contact = F.relu(self.bn_conv_1(self.conv_test_1(infor)))
...
class Lag_PP_mixed(Lag_PP_zero):
    ...
    def __init__(self, steps, k, rho_mode='fix'):
        ...
        # self.rho = 1.0
        self.rho_m = nn.Parameter(torch.randn(500, 500))
        ...
        pos_j, pos_i = np.meshgrid(np.arange(1,500+1)/500.0, 
            np.arange(1,500+1)/500.0)
...
class RNA_SS_e2e(nn.Module):
    ...
    def forward(self, prior, seq, state, embedding):
        u = self.model_att(prior, seq, state, embedding)

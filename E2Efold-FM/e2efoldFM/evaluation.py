...
from e2efoldFM.postprocess import postprocess
...
def all_test_only_e2e(test_generator, contact_net, lag_pp_net, device, test_data):
    ...
    batch_n = 0
    for contacts, seq_embeddings, matrix_reps, seq_lens, names in test_generator:
        if batch_n %10==0:
            print('Batch number: ', batch_n)
        batch_n += 1
        contacts_batch = torch.Tensor(contacts.float()).to(device)
        ...
        # cjy save pred map
        print(a_pred_list[-1])
        print(a_pred_list[-1].shape)
        print(seq_lens)
        print(names)
        pred_map = a_pred_list[-1][0, :seq_lens, :seq_lens]
        import numpy as np
        import os
        save_root_dir = "/data/tanqingxiong/E2EfoldFM_code/results/archive2"
        save_feat_dir = os.path.join(save_root_dir, "output")
        if os.path.exists(save_feat_dir) != True:
            os.makedirs(save_feat_dir)
        np.save(os.path.join(save_feat_dir, names[0]), pred_map)
        ...
        metric_list.append(result_tmp[0])
